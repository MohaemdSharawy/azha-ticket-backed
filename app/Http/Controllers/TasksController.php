...
class TasksController extends Controller
{
    ...
    public function index()
    {
        ...
        $workers  = User::where(['is_worker' => 1])->get();

        return view('tasks.index', compact('hotels', 'status', 'types', 'priority', 'departments', 'workers'));
    }
    ...
    public function assignTaskToWorker(Request $request)
    {
        ...
        if ($task->dep_id == "15") {
            if (!is_null($task->ticket_id)) {
                ...
                // $data =  [
                //     'subject' => 'Assigned  Ticket # ' . $task->ticket_id,
                //     'message' => 'You Assigned FOr Ticket #' . $task->ticket_id . '<br> Waiting Your Response <br> Thank U',
                //     'url'  => $_SERVER["HTTP_HOST"] . '/ticket/show-model/' . $task->ticket_id
                // ];
                // try {
                //     Mail::to($user->email)->send(new DesignRequestEmail($data));
