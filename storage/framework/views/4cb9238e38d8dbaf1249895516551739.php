<script>
    ...
    function getData() {
        ...
        $('#ticket').DataTable({
            ajax: {
                url: "<?php echo e(route('ticket.data')); ?>",
                type: "GET",
                dataType: "json",
                data: {
                    hotels,
    ...
    function ViewTicket(TicketId) {
        ...
        $.ajax({
            url: "<?php echo e(url('ticket/show')); ?>" + '/' + TicketId,
            dataType: "json",
            type: 'GET',
            success: function(data) {
                console.log(data);
    ...
    function buildTicketData(ticket) {
        ...
        if (ticket.description != null) {
            output +=
                `<tr><td style="width: 20%;">
                <i class="fa fa-bolt text-muted mr-1"></i> Details</td><td style="${color}">   <?php echo html_entity_decode('${ticket.description}'); ?></td></tr>`
            // output +=
            //     `<tr><td style="width: 20%;">
        ...
        if (ticket.status_id == 3) {
            action_output +=
                `
            <a class="dropdown-item" href="<?php echo e(url('ticket/update_status/${mainTicket.id}/4')); ?>"><i class="fa fa-handshake"></i> Done</a>
            <a class="dropdown-item" href="<?php echo e(url('ticket/update_status/${mainTicket.id}/2')); ?>"><i class="fa fa-history"> </i> Re-open</a>`
        }
        ...
        if (mainTicket.status_id != 4 && mainTicket.status_id != 3) {
            secoud_action += `
            <a class="dropdown-item" href="<?php echo e(url('ticket/create/${mainTicket.id}')); ?>"> <i class="fa fa-plus-circle"> </i>
                Create Sub Ticket
            </a>
        ...
        function buildAttachment() {
            ...
            if (selected_ticket.attachment.length > 0) {
                ...
                selected_ticket.attachment.forEach(element => {
                    ...
                    <a href="<?php echo e(asset('uploads/attach/${element.file}')); ?>" target="_blank" ><i style="color:#008502;" class="fas fa-file-pdf"></i> Attach # ${count} </a>
                    `)
                })
            }
        }
        ...
        function buildComments() {
            ...
            if (selected_ticket.status_id != 1) {
                $('.comments').append(`
                <a href="<?php echo e(url('ticket_comments/show/${selected_ticket.id}')); ?>">
    ...
    function buildTaskData(task) {
        ...
        if (task.user != null) {
            ...
            if ("<?php echo e(check_create_permission('8')); ?>" && user_department.includes(task.dep_id)) {
        ...
        if ("<?php echo e(check_create_permission('8')); ?>" && user_department.includes(task.dep_id) && check_satatus) {
        ...
        if ([2, 5].includes(task.status_id) && user_department.includes(task
                .dep_id) && "<?php echo e(check_create_permission('8')); ?>") {
            actions +=
                `<br><br><a href="<?php echo e(url('task/change_status/${task.id}/3')); ?>"><button class="btn btn-rounded  btn-info" >Complete</button></a>`
        }
    ...
    function getDepartmentUser(dep_id, task_id, type) {
        ...
        $.ajax({
            url: "<?php echo e(url('department/department_user')); ?>" + '/' + hid + '/' + dep_id,
            type: "GET",
            dataType: "json",
            success: function(data) {
                if (type == "worker") {
    ...
    function addWorker(workers, task_id) {
        $('#assign_user').empty()
        $('#assign_user').append(`
        <form method="POST" action="<?php echo e(route('task.assign.worker')); ?>">
            <?php echo csrf_field(); ?>
    ...
    function createTask(task) {
        ...
        $('.create-task').empty()
        $('.create-task').append(`
            <form action="<?php echo e(route('task.create')); ?>"  method="POST">
                <?php echo csrf_field(); ?>
                <div class="row">
                    ...
                    <input type="hidden" value="<?php echo e(Auth::id()); ?>" name="uid">
                    <div class="col-6">
    ...
    function get_facilities() {
        ...
        let hid = $('#h_id').val();
        $.ajax({
            url: "<?php echo e(route('ticket.facility')); ?>",
            dataType: "json",
            type: 'POST',
            data: {
    ...
    function getServices() {
        ...
        $.ajax({
            url: "<?php echo e(route('ticket.dep_service')); ?>",
            dataType: "json",
            type: 'POST',
            data: {
                "_token": "<?php echo e(csrf_token()); ?>",
...
<?php /**PATH /home/sunrise/public_html/tickets/resources/views/tickets/scripts.blade.php ENDPATH**/ ?>